...
export const adminLearning: Record<string, LearningDefinition> = {
  ...
  '/admin/content': {
    ...
    need: c('Use it when you want to add, edit, publish or remove something but are unsure which section to open.', 'Χρησιμοποιήστε το όταν θέλετε να προσθέσετε, να επεξεργαστείτε, να δημοσιεύσετε ή να αφαιρέσετε κάτι και δεν είστε βέβαιοι ποια ενότητα χρειάζεστε.'),
    steps: [c('Choose a content type.', 'Επιλέξτε τύπο περιεχομένου.'), c('Open an existing item or add a new one.', 'Ανοίξτε ένα υπάρχον στοιχείο ή προσθέστε νέο.'), c('Save, review and publish when ready.', 'Αποθηκεύστε, ελέγξτε και δημοσιεύστε όταν είναι έτοιμο.')],
    after: c('Saved changes remain private until you publish them.', 'Οι αποθηκευμένες αλλαγές παραμένουν ιδιωτικές μέχρι να τις δημοσιεύσετε.'),
    example: {title: c('Update a product', 'Ενημέρωση προϊόντος'), steps: [c('Open Products.', 'Ανοίξτε τα Προϊόντα.'), c('Search and edit the item.', 'Αναζητήστε και επεξεργαστείτε το είδος.'), c('Update the live version.', 'Ενημερώστε τη ζωντανή έκδοση.')], result: c('The same workflow applies to every content type.', 'Η ίδια ροή ισχύει για κάθε τύπο περιεχομένου.')},
  },
  '/admin/pages': {
    label: c('Pages', 'Σελίδες'),
    purpose: c('Adds, edits, publishes and removes website pages from one list.', 'Προσθέτει, επεξεργάζεται, δημοσιεύει και αφαιρεί σελίδες ιστοτόπου από μία λίστα.'),
    need: c('Use it for page content, public URLs and menu placement. Open Visual editor only for layout changes.', 'Χρησιμοποιήστε το για περιεχόμενο σελίδας, δημόσια URLs και τοποθέτηση σε μενού. Ανοίξτε τον Οπτικό επεξεργαστή μόνο για αλλαγές διάταξης.'),
  ...
  '/admin/media': {
    label: c('Gallery', 'Συλλογή'),
    purpose: c('The reusable Gallery for website photos, videos, PDFs and catalogue files, including variants and usage information.', 'Η επαναχρησιμοποιήσιμη Συλλογή φωτογραφιών, βίντεο, PDF και καταλόγων του ιστοτόπου, με παραλλαγές και πληροφορίες χρήσης.'),
    need: c('Use Gallery for files reused by the website. Avoid uploading duplicates; replacing one managed file can preserve its references.', 'Χρησιμοποιήστε τη Συλλογή για αρχεία που επαναχρησιμοποιεί ο ιστότοπος. Αποφύγετε διπλότυπα· η αντικατάσταση ενός αρχείου μπορεί να διατηρήσει τις αναφορές του.'),
