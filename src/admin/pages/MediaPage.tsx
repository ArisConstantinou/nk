...
export function MediaPage() {
  ...
  return <div className="nk-admin-media-page" onDragEnter={event => {if (canWrite && event.dataTransfer.types.includes('Files')) setDragActive(true);}} onDragOver={event => {if (canWrite) event.preventDefault();}} onDrop={onDrop}>
    ...
    <div className="nk-admin-media-toolbar nk-admin-media-toolbar--full"><label><Search/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search filename, tags, folder…" aria-label="Search Gallery"/></label><select value={type} onChange={event => setType(event.target.value)} aria-label="Filter Gallery type"><option value="all">All Gallery items</option><option value="image">Photos & images</option><option value="video">Videos</option><option value="document">PDFs & catalogues</option></select><select value={folder} onChange={event => setFolder(event.target.value)} aria-label="Filter Gallery folder"><option value="all">All folders</option>{folders.map(value => <option key={value}>{value}</option>)}</select><select value={category} onChange={event => setCategory(event.target.value)} aria-label="Filter Gallery category"><option value="all">All categories</option>{categories.map(value => <option key={value}>{value}</option>)}</select><select value={status} onChange={event => setStatus(event.target.value as typeof status)} aria-label="Filter Gallery status"><option value="all">All states</option><option value="active">Active</option><option value="inactive">Inactive</option></select><span>{shown.length} of {items.length}</span></div>
    {loading
      ? <div className="nk-admin-list-loading"><RefreshCw className="nk-admin-spin"/>Loading Gallery…</div>
      : shown.length
        ? <div className="nk-admin-media-grid">{shown.map(item => <article className={!item.active ? 'inactive' : ''} key={item.id}><button className="nk-admin-media-preview" onClick={() => setSelected(item)}><AssetPreview item={item}/></button><span><Folder/> {item.folder} / {item.category}</span><b>{item.title || item.filename}</b><p>{item.caption || item.altText || item.filename}</p><footer><small>{(item.size / 1024 / 1024).toFixed(item.size > 1024 * 1024 ? 1 : 2)} MB{item.width ? ` · ${item.width}×${item.height}` : ''} · {item.variants.length} variants</small><ActionMenu compact placement="top" label={`Actions for ${item.filename}`}><button role="menuitem" onClick={() => void copyUrl(item)}><Link2/>Copy URL</button>{canWriteItem(item) && <button role="menuitem" onClick={() => void duplicate(item)} disabled={busy}><Copy/>Duplicate</button>}{user?.role === 'owner' && <button role="menuitem" className="danger" onClick={() => void remove(item)} disabled={busy}><Trash2/>Delete</button>}</ActionMenu></footer></article>)}</div>
        : <EmptyState title="No Gallery items found" body={query || status !== 'all' || type !== 'all' || folder !== 'all' || category !== 'all' ? 'Change or clear the filters to see more items.' : canWrite ? 'Drop files here or upload the first Gallery item.' : 'No shared Gallery files have been uploaded.'}/>}
