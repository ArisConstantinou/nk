...
export function NavigationPage({embedded = false, requestedItemId = '', requestedNewMenu = null, onItemsChange, onClose}: NavigationPageProps = {}) {
  ...
  return <div className={embedded ? 'nk-admin-navigation-embedded' : ''}>
    {embedded
      ...
      : <PageHeading
        eyebrow="SITE STRUCTURE / MENUS"
        title="Navigation menus"
        description="Control desktop mega menus, mobile navigation and footer links from one ordered source."
        actions={<><Link to="/admin/pages"><FileText/>Pages</Link>{canWrite && <button className="nk-admin-primary" data-guide="new-navigation-link" onClick={()=>setEditing(emptyItem(menu))}><Plus/>Add link</button>}</>}
      />}
