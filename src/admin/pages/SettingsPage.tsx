...
export function SettingsPage() {
  ...
  return <div className="nk-settings-page">
    ...
    <div className="nk-settings-toolbar"><div className={`nk-settings-state ${dirty ? 'draft' : 'saved'}`}>{dirty ? <Megaphone/> : <Check/>}<span><b>{dirty ? 'Unsaved draft' : record.status === 'published' ? 'Saved and published' : 'Draft saved'}</b><small>Version {record.version} · live preview updates immediately</small></span></div><div className="nk-settings-devices" aria-label="Preview device">{(['desktop', 'tablet', 'mobile'] as const).map(value => <button className={device === value ? 'active' : ''} onClick={() => setDevice(value)} key={value}>{value}</button>)}</div>{canWrite && <><button onClick={() => void save()} disabled={busy || !dirty}><Save/>{busy ? 'Working…' : 'Save draft'}</button><button className="nk-admin-primary" onClick={() => void publish()} disabled={busy}><Rocket/>Publish</button></>}</div>
    <div className="nk-settings-workspace">
      <aside><nav aria-label="Settings sections">{tabs.map(item => <button className={tab === item.id ? 'active' : ''} onClick={() => setTab(item.id)} key={item.id}><item.icon/><span>{item.label}</span><ChevronRight/></button>)}</nav><p><Globe2/><span><b>Draft-safe preview</b>Your edits appear on the right now. Visitors see them only after publishing.</span></p></aside>
      <main>
