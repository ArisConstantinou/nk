...
export function AdminLayout() {
  ...
  const customerDestination = user ? canReadForms(user.role) ? '/admin/forms' : canManageEnquiries(user.role) ? '/admin/enquiries' : null : null;
  const toolsDestination = user ? canReadKind(user.role, 'settings') ? '/admin/settings' : canManageInteractive(user.role) ? '/admin/interactive' : canReadKind(user.role, 'seo') ? '/admin/seo' : null : null;
  useEffect(() => {
  ...
  return <div className="nk-admin-shell">
    ...
    <section ref={workspaceRef} className="nk-admin-workspace">
      <header className="nk-admin-topbar"><NavLink className="nk-admin-mobile-home-logo" to="/admin/dashboard" onClick={() => {closeCommand('dismiss'); setMobileOpen(false); setExplorerOpen(false); setQuickAddOpen(false);}} aria-label="Home"><img src={publicAsset('assets/nk-logo-transparent-v2.png')} alt="NK Electrical"/></NavLink><button ref={mobileTriggerRef} className="nk-admin-menu-trigger" type="button" onClick={() => setMobileOpen(true)} aria-label="Open admin navigation" aria-expanded={mobileOpen} aria-controls="admin-navigation"><Menu/></button><nav aria-label="Breadcrumb"><NavLink to="/admin/dashboard">Admin</NavLink><ChevronRight/><span>{currentGroup}</span><ChevronRight/><b>{currentLabel}</b></nav><div className="nk-admin-topbar-actions"><button className="nk-admin-mobile-help-button" type="button" onClick={openGuide} aria-label="Help for this screen"><HelpCircle/></button><Link className="nk-admin-mobile-website-button" to="/?liveEdit=1" onClick={() => {closeCommand('dismiss'); setMobileOpen(false); setExplorerOpen(false); setQuickAddOpen(false);}} aria-label={text('View live website', 'Προβολή ζωντανής ιστοσελίδας')}><ExternalLink/></Link><button ref={mobileNavTriggerRef} className="nk-admin-mobile-account-button" type="button" onClick={() => {closeCommand('dismiss'); setExplorerOpen(false); setQuickAddOpen(false); setMobileOpen(true);}} aria-label="Open account and settings" aria-expanded={mobileOpen} aria-controls="admin-navigation"><span className="nk-admin-mobile-account-initials">{user.displayName.split(/\s+/).slice(0,2).map(part => part[0]).join('').toUpperCase()}</span></button><button className="nk-admin-topbar-guide" type="button" onClick={openGuide} aria-label="Open Guide / Οδηγός"><HelpCircle/><span className="nk-admin-guide-label-full">Guide / Οδηγός</span><span className="nk-admin-guide-label-compact">Guide</span></button><button ref={commandTriggerRef} className={`nk-admin-global-search ${commandOpen && commandDocked ? 'active' : ''}`} type="button" aria-label="Search admin" onClick={() => commandOpen && commandDocked ? closeCommand('dismiss') : openCommand(window.matchMedia('(max-width: 840px)').matches)} aria-expanded={commandOpen && commandDocked} aria-controls="admin-command-panel"><Search/><span>Search</span><kbd>Ctrl K</kbd></button><NavLink className="nk-admin-site-edit-link" to="/?liveEdit=1" aria-label="Visit the live site in edit mode"><span>Visit site</span><ExternalLink/></NavLink>{isPagesAdminMode ? <span className="nk-admin-topbar-avatar" aria-label="Mobile device admin">{user.displayName.split(/\s+/).slice(0,2).map(part => part[0]).join('').toUpperCase()}</span> : <NavLink className="nk-admin-topbar-avatar" to="/admin/profile" aria-label="Open your profile">{user.displayName.split(/\s+/).slice(0,2).map(part => part[0]).join('').toUpperCase()}</NavLink>}</div></header>
      <div className={`nk-admin-mobile-subnav-slot${commandOpen && commandDocked ? ' is-search-open' : ''}`}>
        {commandOpen && commandDocked ? <CommandPalette open onClose={closeCommand} role={user.role} fallbackFocusRef={mobileSearchTriggerRef} guided={false} docked/>
          : <nav className="nk-admin-mobile-subnav" aria-label="Admin sections">
            <NavLink to="/admin/content" onClick={() => {setExplorerOpen(false); setQuickAddOpen(false);}} className={({isActive}) => isActive || mobileEditActive || location.pathname === '/admin/dashboard' ? 'active' : ''}><FileText/><span>Content</span></NavLink>
            {customerDestination ? <NavLink to={customerDestination} onClick={() => {setExplorerOpen(false); setQuickAddOpen(false);}}><Users/><span>Customers</span></NavLink> : <span aria-hidden="true"/>}
            {toolsDestination ? <NavLink to={toolsDestination} onClick={() => {setExplorerOpen(false); setQuickAddOpen(false);}}><Settings/><span>Tools</span></NavLink> : <span aria-hidden="true"/>}
            <NavLink to="/admin/audit" onClick={() => {setExplorerOpen(false); setQuickAddOpen(false);}}><Activity/><span>Activity</span></NavLink>
          </nav>}
      </div>
      ...
      {quickAddOpen && <section id="admin-quick-add" className="nk-admin-quick-add" aria-label="Create content"><header><div><Plus/><span><b>Create content</b><small>Choose exactly what you are creating</small></span></div><button type="button" onClick={() => setQuickAddOpen(false)} aria-label="Close create menu"><X/></button></header><div>{canWriteKind(user.role, 'page') && <Link to="/admin/pages?new=1" onClick={() => setQuickAddOpen(false)}><FileText/><span><b>Create page</b><small>Create one website page</small></span></Link>}{canWriteKind(user.role, 'product') && <Link to="/admin/products?new=1" onClick={() => setQuickAddOpen(false)}><ShoppingBag/><span><b>Create product</b><small>Create one shop product</small></span></Link>}{canWriteKind(user.role, 'service') && <Link to="/admin/services?new=1" onClick={() => setQuickAddOpen(false)}><BriefcaseBusiness/><span><b>Create service</b><small>Create one customer service</small></span></Link>}{canWriteKind(user.role, 'project') && <Link to="/admin/projects?new=1" onClick={() => setQuickAddOpen(false)}><FolderKanban/><span><b>Create project</b><small>Create one completed project</small></span></Link>}{canWriteKind(user.role, 'catalogue') && <Link to="/admin/catalogues?new=1" onClick={() => setQuickAddOpen(false)}><BookOpen/><span><b>Create catalogue</b><small>Create one catalogue record</small></span></Link>}</div><footer><Image/><span>Photos, videos and PDFs are uploaded only from <b>Gallery</b>.</span></footer></section>}
    </section>
    <nav className="nk-admin-mobile-nav" aria-label={text('Mobile admin navigation', 'Κύρια πλοήγηση διαχείρισης')}>
      ...
      <button type="button" className="nk-admin-mobile-add" onClick={() => {closeCommand('dismiss'); setMobileOpen(false); setExplorerOpen(false); setQuickAddOpen(open => !open);}} aria-label={text('Create content', 'Δημιουργία περιεχομένου')} aria-expanded={quickAddOpen} aria-controls="admin-quick-add"><Plus/><span>{text('Create', 'Δημιουργία')}</span></button>
    ...
    <CommandPalette open={commandOpen && !commandDocked} onClose={closeCommand} role={user.role} fallbackFocusRef={commandTriggerRef} guided={false}/>
