...
export const electricalInstallationTemplate: ExperienceDocument = {
  ...
  title: 'From site review to tested handover',
  description: 'Follow an electrical installation from the initial site review and setting out through first fix, final connections, testing and handover.',
  stage: {
  ...
  sections: [
    section('section-empty-wall', 'Confirm the site condition', 'Before site work, we review the room, drawings, supply and intended use.'),
    section('section-marking', 'Survey, measure and mark', 'We confirm accessory positions, cable routes, load requirements and coordination points on site.', [...sprayMarks, worker('worker-marking', 'ELECTRICIAN · MEASURE + SPRAY')]),
    section('section-wall-chases', 'Prepare containment routes', 'Wall chases and box recesses are formed to the approved layout before first fix.', [...chases, worker('worker-chases', 'BUILDER · WALL CHASER')]),
    section('section-bed-boxes', 'Install boxes and supports', 'Back boxes, cabinet positions and supports are secured at the correct heights and locations.', boxes),
    section('section-fit-conduit', 'Install conduit and containment', 'Conduit and containment are routed between distribution, control points and final outlets.', conduits),
    section('section-pull-cables', 'Pull and identify conductors', 'Conductors are installed through the prepared routes, identified and prepared for termination.', cables),
    section('section-repair-wall', 'Inspect before closing walls', 'The first fix is checked against the agreed layout before chases are repaired and finishes restored.'),
    section('section-cabinet', 'Install the control cabinet', 'Drivers, supplies and connections are placed in an accessible cabinet for testing and maintenance.', [cabinet]),
    section('section-shelves', 'Coordinate final fittings', 'Final fittings and joinery are positioned so electrical points, access and clearances remain correct.', [cabinet, ...shelfLayers, lowerShelf]),
    section('section-aluminium', 'Fit lighting profiles', 'Aluminium profiles are fitted to the agreed detail, ready for the specified lighting system.', [cabinet, ...shelfLayers, lowerShelf, ...shelfLayers.map((item, index) => layer({id: `profile-${index + 1}`, name: `Aluminium profile ${index + 1}`, type: 'rectangle', x: item.transform.x + 30, y: item.transform.y + 53, width: item.transform.width - 60, height: 12, fill: '#bcc5c7', stroke: '#e1e7e8', strokeWidth: 2}))]),
    section('section-led-strips', 'Install and connect LED strips', 'LED strips, drivers and connections are installed to the specified circuit and control arrangement.', [cabinet, ...shelfLayers, lowerShelf, ...shelfLayers.map((item, index) => layer({id: `led-${index + 1}`, name: `Shelf LED ${index + 1}`, type: 'line', x: item.transform.x + 38, y: item.transform.y + 59, width: item.transform.width - 76, height: 1, stroke: '#f0c64c', strokeWidth: 8})), layer({id: 'led-lower', name: 'Lower shelf LED', type: 'line', x: 700, y: 958, width: 960, height: 1, stroke: '#f0c64c', strokeWidth: 8})]),
    section('section-double-switch', 'Complete the second fix', 'Switches, accessories and final connections are fitted, labelled and checked before energisation.', [cabinet, ...shelfLayers, lowerShelf, layer({id: 'double-switch', name: 'Double switch placeholder', type: 'placeholder', x: 330, y: 450, width: 180, height: 120, fill: '#f3f1eb', stroke: '#a9a49c', strokeWidth: 4, text: 'DOUBLE SWITCH'}), worker('worker-switch', 'ELECTRICIAN · DRILL SWITCH')]),
    section('section-power-on', 'Test, demonstrate and hand over', 'Circuits and controls are inspected and tested; operation is demonstrated before handover.', [cabinet, ...shelfLayers, lowerShelf, ...shelfLayers.map((item, index) => layer({id: `powered-led-${index + 1}`, name: `Powered shelf LED ${index + 1}`, type: 'line', x: item.transform.x + 38, y: item.transform.y + 59, width: item.transform.width - 76, height: 1, stroke: '#fff1a8', strokeWidth: 18})), layer({id: 'powered-led-lower', name: 'Powered lower LED', type: 'line', x: 700, y: 958, width: 960, height: 1, stroke: '#fff1a8', strokeWidth: 18}), layer({id: 'double-switch-final', name: 'Interactive double switch', type: 'placeholder', x: 330, y: 450, width: 180, height: 120, fill: '#f3f1eb', stroke: '#a9a49c', strokeWidth: 4, text: 'SWITCH 1 · SWITCH 2'})]),
  ],
};
