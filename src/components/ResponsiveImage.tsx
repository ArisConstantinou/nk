...
export function ResponsiveImage({src = '', alt, ...props}: ImgHTMLAttributes<HTMLImageElement>) {
  ...
  if (!variants.length) return <img src={resolvedSrc} alt={alt} {...props}/>;
  const srcSet = variants.map(item => `${resolvePublicUrl(item.url)} ${item.width}w`).join(', ');
  return <picture style={{display: 'contents'}}><source type="image/webp" srcSet={srcSet} sizes={props.sizes || '100vw'}/><img src={resolvedSrc} alt={alt ?? asset?.altText ?? ''} width={asset?.width || undefined} height={asset?.height || undefined} {...props}/></picture>;
}
