  ...
  >
    ...
    <header className={`ia-header ${settings.header.sticky ? '' : 'ia-header--static'} ${showHomeHeaderPreview ? 'ia-header--home-preview' : ''}`} ref={headerRef}>
      ...
      <div className="ia-header-bar">
        <Link className={`ia-brand ${showHomeHeaderPreview ? 'ia-brand--home-preview' : ''}`} to="/" {...routeLinkAttributes('/')} aria-label={`${settings.brandName} home`} aria-hidden={mobileOpen || undefined} tabIndex={mobileOpen ? -1 : undefined}><span className="ia-brand-mark"><ResponsiveImage className="ia-brand-logo" src={settings.logoUrl || publicAsset('assets/nk-logo-transparent-v2.png')} alt={settings.logoAlt}/></span><span className="ia-brand-copy"><strong><span className="ia-brand-depth" aria-hidden="true">{railBrandLabel}</span><span className="ia-brand-face" data-visual-kind="settings" data-visual-slug="business-details" data-visual-path="brandName" data-visual-edit="text" data-visual-label="Brand name">{railBrandLabel}</span></strong></span></Link>
