...
function BuilderComponent({component, pageSlug, section, sectionIndex, index}: {component: PublicPageComponent; pageSlug: string; section: PublicPageSection; sectionIndex: number; index: number}) {
  ...
  else if (component.type === 'gallery') content = <div className="cms-component-gallery" aria-label={component.label || 'Image gallery'}>{component.images.length ? component.images.map((image, imageIndex) => <div key={`${component.id}-${imageIndex}`}><ResponsiveImage src={image} alt={component.alt ? `${component.alt} ${imageIndex + 1}` : `Gallery image ${imageIndex + 1}`}/></div>) : <span className="cms-component-placeholder">Choose at least two gallery images</span>}</div>;
